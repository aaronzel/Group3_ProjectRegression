...
data %>% group_by(is_homeowner) %>% summarize(mean_rate = mean(borrower_rate))%>%  ggplot(aes(is_homeowner,mean_rate, fill = is_homeowner)) + geom_col() + xlab("Homeowner Status") + ylab("Average Interest Rate") + ggtitle("Average Interest Rate by Homeowner Status") + guides(fill = FALSE) + theme(plot.title = element_text(hjust = 0.5))

# Graph of Home Ownership and Default
data$default_numeric <- as.numeric(data$default)
 data %>% group_by(is_homeowner) %>% summarize(mean_default = mean(default))%>%  ggplot(aes(is_homeowner,mean_default, fill = is_homeowner)) + geom_point()  + xlab("Homeowner Status") + ylab("Average Default Rate") + ggtitle("Average Default Rate by Homeowner Status") + guides(fill = FALSE) + theme(plot.title = element_text(hjust = 0.5))
```
