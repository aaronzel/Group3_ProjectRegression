...
*Groups we would target*

Those with current delinquencies

Those with higher bankcard utilization

Those with a prosper rating below AA 

Those with a higher listing category id

*Groups we would avoid*

Homeowners

Those asking for relatively large amounts of capital

Those with several inquiries

Those with AA prosper rating

Those with a greater principal balance

### Arbitrage Analysis

**Arbitrage Analysis**

We segmented the opportunities for arbitrage into four different categories. 
...
Category 3: Variables that lower interest rate but raise default rate

Category 4: Variables that raise interest rate but lower default rate
<br />
...
# Understand homeowner variable 
str(data$is_homeowner)
...
data %>% group_by(is_homeowner) %>% summarize(mean_rate = mean(borrower_rate))%>%  ggplot(aes(is_homeowner,mean_rate, fill = is_homeowner)) + geom_col() + xlab("Is Homeowner") + ylab("Average Borrower Rate") + ggtitle("Average Borrower Rate - Homeowner Status") + guides(fill = FALSE) + theme(plot.title = element_text(hjust = 0.5))

# Graph of Home Ownership and Default
data$default_numeric <- as.numeric(data$default)
 data %>% group_by(is_homeowner) %>% summarize(mean_default = mean(default))%>%  ggplot(aes(is_homeowner,mean_default, fill = is_homeowner)) + geom_point()  + xlab("Is Homeowner") + ylab("Average Default Rate") + ggtitle("Average Default Rate - Homeowner Status") + guides(fill = FALSE) + theme(plot.title = element_text(hjust = 0.5))
...
Table Showing Average Interest Rate by Homeowner Status:
```{r}
...
Table Showing Average Default Rate by Homeowner Status: 
```{r}
...
<br />
These tables support the conclusions evidenced by the above graphs. We see that homeowners pay 17.3% interest on average, while non-home owners pay 19.5% on average. This 2.2% difference is not justified by the 1% difference in default rate (Homeowners default 5.2% of the time while non-home owners default 0.42% of the time). 
