...
``` {r}
number_of_days_interest <- ggplot(data = data, aes(number_of_days, borrower_rate)) + geom_bin2d(bins = 25) + scale_fill_continuous(type = "viridis") + xlab("Number of Days") + ylab("Borrower Rate") + ggtitle("Borrower rate vs. Number of Days") + xlim(816, 971.2)
number_of_days_interest
```

As we can see from our density plot, there is no clear pattern between number of days and interest rates besides a slight lean towards shorter terms which was not significant enough to be picked up by our linear model
...
As we can see from our line of fit, the default rate has a hike at around 450 days. Even though it decreases at around 700 days, there is an observable pattern in this data.
