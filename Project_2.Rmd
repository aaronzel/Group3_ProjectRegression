...
default_numeric <- as.numeric(data$default)
 data %>% group_by(is_homeowner) %>% summarize(mean_default = mean(default_numeric))%>%  ggplot(aes(is_homeowner,mean_default, fill = is_homeowner)) + geom_point()  + xlab("Is Homeowner") + ylab("Average Default Rate") + ggtitle("Average Default Rate - Homeowner Status") + guides(fill = FALSE) + theme(plot.title = element_text(hjust = 0.5))

#  
 
 
 
# mean_rate_owner <- data %>% group_by(is_homeowner) %>% summarize(mean_rate = mean(borrower_rate))
 
# mean_default <- data %>% group_by(is_homeowner) %>% summarize(mean_default = mean(default_numeric))
